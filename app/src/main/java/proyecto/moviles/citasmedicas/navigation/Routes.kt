package proyecto.moviles.citasmedicas.navigation

// Rutas centralizadas para facilitar la incorporación de nuevas pantallas.
object Routes {
    const val SPLASH = "splash"
    const val ONBOARDING_1 = "onboarding_1"
    const val ONBOARDING_2 = "onboarding_2"
    const val ONBOARDING_3 = "onboarding_3"
    const val LOGIN = "login"
    const val RECOVER_PASSWORD = "recover_password"
    const val REGISTER = "register"
    const val PATIENT_HOME = "patient_home"
    const val APPOINTMENT_HISTORY = "appointment_history"
    const val USER_PROFILE = "user_profile"
    const val SEARCH_DOCTOR = "search_doctor"
    const val SCHEDULE_DETAILS = "schedule_details"
    const val PATIENT_APPOINTMENT_DETAIL = "patient_appointment_detail"
    const val DOCTOR_HOME = "doctor_home"
    const val DOCTOR_APPOINTMENT_DETAIL = "doctor_appointment_detail"
    const val DOCTOR_AVAILABILITY = "doctor_availability"
    const val DOCTOR_PROFILE = "doctor_profile"
}

